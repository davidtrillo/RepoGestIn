...
$app->get('/api/12_100/activas/{instalacion}', function (Request $request, Response $response) {

    $instalacion = $request->getAttribute('instalacion');
    $sql = 'SELECT count(id) AS c FROM 12_100 WHERE activo="true" AND idInstalacion="' . $instalacion . '"';
    try {
        $db = new db();
        $db = $db->conectDB();
        $resultado = $db->prepare($sql);
...
$app->post('/api/12_100/nueva', function (Request $request, Response $response) {
    ...
    $sql = 'INSERT INTO 12_100 (id, idInstalacion, idTipoActuacion, idNumSerie, idUsuario,albaran, nid, observaciones, fechaActuacion, precio, activo) VALUES (NULL, :idInstalacion, :idTipoActuacion, :idNumSerie, :idUsuario,:albaran , :nid,:observaciones, :fechaActuacion, :precio, :activo);';
    // $sql='INSERT INTO 12_100 (idInstalacion) VALUES (:idInstalacion);';
...
$app->delete('/api/12_100/borrar/{id}', function (Request $request, Response $response) {

    $id = $request->getAttribute('id'); // PARA RECUPERAR LA ID DEL REGISTRO QUE SE VA A HACER UPDATE

    $sql = 'DELETE FROM 12_100 WHERE id=' . $id;
...
$app->put('/api/12_100/modificar/{id}', function (Request $request, Response $response) {
    //declaracion de las variables de recepcion desde FRONT
    ...
    //  $sql='UPDATE 12_100 SET idTipoActuacion=:idtipoActuacion,idNumSerie=:idNumSerie,idUsuario=:idUsuario,observaciones=:observaciones,fechaActuacion=:fechaActuacion,precio=:precio,activo=:activo WHERE id='.$id;
    $sql = 'UPDATE 12_100 SET albaran=:albaran,nid=:nid,idTipoActuacion=:idTipoActuacion,idNumSerie=:idNumSerie,idUsuario=:idUsuario,observaciones=:observaciones, fechaActuacion=:fechaActuacion,precio=:precio,activo=:activo WHERE id='. $id;
