...
$app->get('/api/12_bici', function (Request $request, Response $response) {
    ...
    $sql = 'SELECT t.id, ta.descripcion,t.idNumSerie,t.albaran,t.observaciones,t.fechaActuacion,t.precio,t.activo FROM 12_bici t inner join tipoactuacion ta on t.idTipoActuacion=ta.id order by t.fechaActuacion desc';
    try {
        $db = new db();
        $db = $db->conectDB();
...
$app->get('/api/12_bici/{instalacion}', function (Request $request, Response $response) {
    ...
    $instalacion = $request->getAttribute('instalacion');
    $sql = 'SELECT t.id,t.idTipoActuacion,ta.descripcion,t.idNumSerie,t.albaran,t.observaciones,t.fechaActuacion,t.precio,t.activo FROM 12_bici t inner join tipoactuacion ta on t.idTipoActuacion=ta.id WHERE idInstalacion="' . $instalacion . '" order by t.fechaActuacion desc';
    try {
        $db = new db();
        $db = $db->conectDB();
        $resultado = $db->prepare($sql);
