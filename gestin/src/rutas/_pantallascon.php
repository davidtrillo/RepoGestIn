...
$app->get('/api/pantallascon', function (Request $request, Response $response) {
    ...
    $sql = 'SELECT t.id,  t.idTipoActuacion,t.idNumSerie,t.albaran,t.observaciones,t.fechaActuacion,t.precio,t.activo FROM PantallasCon t   order by t.activo desc,t.fechaActuacion desc';
    try {
        $db = new db();
        $db = $db->conectDB();
...
$app->get('/api/pantallascon/{instalacion}', function (Request $request, Response $response) {
    ...
    $instalacion = $request->getAttribute('instalacion');
    $sql = 'SELECT t.id,t.idTipoActuacion, t.idTipoActuacion,t.idNumSerie,t.albaran,t.observaciones,t.fechaActuacion,t.precio,t.activo FROM PantallasCon t   WHERE idInstalacion="' . $instalacion . '" order by t.activo desc,t.fechaActuacion desc';
    try {
        $db = new db();
        $db = $db->conectDB();
        $resultado = $db->prepare($sql);
...
$app->post('/api/pantallascon/nueva', function (Request $request, Response $response) {
    ...
    try {
        ...
        $resultado->execute();
        echo json_encode("Pantalla guardada con éxito", JSON_UNESCAPED_UNICODE);
...
$app->delete('/api/pantallascon/borrar/{id}', function (Request $request, Response $response) {
    ...
    try {
        ...
        if ($resultado->rowCount() > 0) {
            ...
            echo json_encode("Pantalla eliminada con éxito", JSON_UNESCAPED_UNICODE);
...
$app->put('/api/pantallascon/modificar/{id}', function (Request $request, Response $response) {
    //declaracion de las variables de recepcion desde FRONT
    ...
    try {
       ...
       // $resultado->bindParam(':id',$id);
        ...
        $resultado->execute();
        echo json_encode("Pantalla editada con éxito", JSON_UNESCAPED_UNICODE);
