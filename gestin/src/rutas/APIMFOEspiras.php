...
//POST para modificar instalacion UPDATE BY ID
 ...
 $app->get('/api/mfoespiras/imprimir/{a}/{b}',function(Request $request, Response $response){
    ...
    $sql='SELECT idInstalacion, i.ubicacion, fechaActuacion, observaciones, precio FROM mfoespiras m JOIN instalaciones i ON m.idInstalacion=i.id WHERE month(m.fechaactuacion)="'.$mes.'" AND year(m.fechaactuacion)="'.$año.'"';
