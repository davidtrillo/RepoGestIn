...
$app->get('/api/13_322', function (Request $request, Response $response) {
    ...
    $sql = 'SELECT t.id,  t.idTipoActuacion,t.idNumSerie,t.albaran,t.observaciones,t.fechaActuacion,t.precio,t.activo FROM 13_322 t   order by t.activo desc,t.fechaActuacion desc';
    try {
        $db = new db();
        $db = $db->conectDB();
...
$app->get('/api/13_322/{instalacion}', function (Request $request, Response $response) {
    ...
    $instalacion = $request->getAttribute('instalacion');
    $sql = 'SELECT t.id,t.idTipoActuacion, t.idTipoActuacion,t.idNumSerie,t.albaran,t.nid,t.observaciones,t.fechaActuacion,t.precio,t.activo FROM 13_322 t   WHERE idInstalacion="' . $instalacion . '" order by t.activo desc,t.fechaActuacion desc';
    try {
        $db = new db();
        $db = $db->conectDB();
        $resultado = $db->prepare($sql);
...
$app->post('/api/13_322/nueva', function (Request $request, Response $response) {
    ...
    $sql = 'INSERT INTO 13_322 (id, idInstalacion, idTipoActuacion, idNumSerie, idUsuario,albaran, nid, observaciones, fechaActuacion, precio, activo) VALUES (NULL, :idInstalacion, :idTipoActuacion, :idNumSerie, :idUsuario,:albaran , :nid,:observaciones, :fechaActuacion, :precio, :activo);';
    ...
    try {
        ...
        $resultado->execute();
        echo json_encode("13-322 guardado con éxito", JSON_UNESCAPED_UNICODE);
...
$app->delete('/api/13_322/borrar/{id}', function (Request $request, Response $response) {
    ...
    try {
        ...
        if ($resultado->rowCount() > 0) {
            ...
            echo json_encode("13-322 eliminado con éxito", JSON_UNESCAPED_UNICODE);
...
$app->put('/api/13_322/modificar/{id}', function (Request $request, Response $response) {
    ...
    $sql = 'UPDATE 13_322 SET albaran=:albaran,nid=:nid,idTipoActuacion=:idTipoActuacion,idNumSerie=:idNumSerie,idUsuario=:idUsuario,observaciones=:observaciones, fechaActuacion=:fechaActuacion,precio=:precio,activo=:activo WHERE id='. $id;
    ...
    try {
       ...
       // $resultado->bindParam(':id',$id);
        ...
        $resultado->execute();
        echo json_encode("13-322 editado con éxito", JSON_UNESCAPED_UNICODE);
