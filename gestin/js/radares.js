...
function nuevaRadares() {
    ...
    if (idInstalacion.value != "") {
        ...
        var activo = document.getElementById('inputActivoRadar').checked;
        var instalada = document.getElementById('inputInstaladaRadar').checked;
        var almacen = null;
        var residuos = null;
...
async function formRadares(elemento) {
    ...
    if (instalacion.value != "") {
        ...
        <!-- ALERTAAAAA ESTÁ AL REVES PERO FUNCIONA ASÍ POR NO CAMBIAR TODO EL CÓDIGO!!! INSTALADA ES ACTIVO Y ACTIVO ES INSTALADA -->
            <input type="checkbox" class=" mt-3 ml-2" name="inputInstaladaRadar" id="inputInstaladaRadar" onclick="checkRadarInstalada()"> 
            <input type="checkbox" class=" mt-3 ml-2" name="inputActivoRadar" id="inputActivoRadar" onclick="checkRadarActiva()">
        </div>  
        <div class="col-1">
            ...
            if (response == "No se han encontrado resultados") {
              ...
              //  console.log("residuos:"+response[i]['residuos'].value);
                ...
                for (var i in response) {
                 ...
                 <div class="col-1">
                    <input type="checkbox" class=" mt-3 ml-2" name="" id="inputActivoRadarTar${response[i]['id']}" onclick="checkRadarActiva(${response[i]['id']})" ${activo}>
                    <input type="checkbox" class=" mt-3 ml-2" name="" id="inputInstaladaRadarTar${response[i]['id']}" onclick="checkRadarInstalada(${response[i]['id']})"  ${instalada}>
                    <input type="checkbox" class=" mt-3 ml-3" name="" id="inputAlmacenRadarTar${response[i]['id']}" onclick="checkRadarAlmacen(${response[i]['id']})"  ${almacen}>
                    <input type="checkbox" class=" mt-3 ml-2" name="" id="inputResiduosRadarTar${response[i]['id']}" onclick="checkRadarResiduos(${response[i]['id']})" ${residuos}>                 
                </div>
                 <div class="col-1">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editarRadares(this.id)" title="Guardar edición"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" title="Eliminar registro" id="${response[i]['id']}" onclick="borrarRadares(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
...
function editarSimpleRadares(param) {
    ...
    var inputActivoTar = document.getElementById('inputActivoRadarTar' + param).checked;
    var inputInstaladaTar = document.getElementById('inputInstaladaRadarTar' + param).checked;
    var inputAlmacenTar = document.getElementById('inputAlmacenRadarTar' + param).checked;
    var inputResiduosTar = document.getElementById('inputResiduosRadarTar' + param).checked;
...
async function editarRadares(param) {


    if  (document.getElementById('inputAlmacenRadarTar' + param).checked){
        ...
        if  (document.getElementById('inputResiduosRadarTar' + param).checked){
...
document.addEventListener("DOMContentLoaded", async function(event) {
   ...
   await checkRadarInstalada();
   await checkRadarActiva();
   await checkRadarAlmacen();
   await checkRadarResiduos();
    // Aquí puedes escribir el código adicional que quieres que se ejecute cuando se dispara el evento DOMContentLoaded
  });
*/
function checkRadarInstalada(id) {

    if (id){

        if (document.getElementById('inputInstaladaRadarTar'+id).checked) {
          //  document.getElementById('inputActivoTar'+id).checked=true;
            document.getElementById('inputAlmacenRadarTar'+id).checked=false;
            document.getElementById('inputResiduosRadarTar'+id).checked=false;
        
        }else{
            document.getElementById('inputActivoRadarTar'+id).checked=false;
            document.getElementById('inputInstaladaRadarTar'+id).checked=false;
            document.getElementById('inputAlmacenRadarTar'+id).checked=false;
            document.getElementById('inputResiduosRadarTar'+id).checked=false;

        }
    }else{
        if (document.getElementById('inputInstaladaRadar').checked) {
            document.getElementById('inputActivoRadar').checked=true;
      

        
        }else{
            document.getElementById('inputActivoRadar').checked=false;
            document.getElementById('inputInstaladaRadar').checked=false;
...
function checkRadarActiva(id) {

    if (id){


        if (document.getElementById('inputActivoRadarTar'+id).checked) {
        
            document.getElementById('inputInstaladaRadarTar'+id).checked=true;
            document.getElementById('inputAlmacenRadarTar'+id).checked=false;
            document.getElementById('inputResiduosRadarTar'+id).checked=false;      
        ...
        if (document.getElementById('inputActivoRadar').checked) {
...
function checkRadarAlmacen(id) {

    if (id){

        if ( document.getElementById('inputAlmacenRadarTar'+id).checked) {
            document.getElementById('inputActivoRadarTar'+id).checked=false;
            document.getElementById('inputInstaladaRadarTar'+id).checked=false;
            document.getElementById('inputResiduosRadarTar'+id).checked=false;
            ...
            if ( document.getElementById('inputAlmacenRadar').checked) {
                document.getElementById('inputActivoRadar').checked=false;
                document.getElementById('inputInstaladaRadar').checked=false;
...
function checkRadarResiduos(id) {

    if (id){

        if (document.getElementById('inputResiduosRadarTar'+id).checked) {
        ...
        if (document.getElementById('inputResiduosRadar').checked) {
