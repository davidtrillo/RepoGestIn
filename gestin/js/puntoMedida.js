...
function nuevaPuntoMedida() {
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFechaPuntoMedida(fechaActuacion)) {
            if (existeFechaPuntoMedida(fechaActuacion)) {
...
function validarFormatoFechaPuntoMedida(campo) {
...
function existeFechaPuntoMedida(fecha) {
...
function existeFecha2PuntoMedida(fecha) {
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
async function formPuntoMedida(elemento) {
    ...
    if (instalacion.value != "") {
        ...
        <div class="col-1">
            <input type="text" class="form-control mt-1" name="inputNumSerie" id="inputNumSerie" onfocusout="comprobarNumSeriePuntoMedida()">
        </div>
        <div class="col-1">
        ...
        await rellenarTodosPuntoMedida();
    }
}



async function rellenarTodosPuntoMedida() { //Llamada a la API 
    var idInstalacion = document.getElementById('inputInstalacion').value;

    var url = 'http://172.27.120.120/gestin/public/api/puntomedida/' + idInstalacion
  await  fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
   ...
   await rellenarFooterPuntoMedida();
   await comprobarNumSeriePuntoMedida2();


}

function rellenarFooterPuntoMedida() {
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/puntomedida/instaladas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>PuntoMedida</b> Instaladas: ${response[0]['c']}</span>
                `
            }
        })
...
function borrarPuntoMedida(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://172.27.120.120/gestin/public/api/puntomedida/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editarPuntoMedida(param) {
    ...
    //validar fecha correcta
    if (validarFormatoFechaPuntoMedida(inputFechaActuacionTar)) {
        if (existeFechaPuntoMedida(inputFechaActuacionTar)) {
    ...
    var url = 'http://172.27.120.120/gestin/public/api/puntomedida/modificar/' + param;
...
function comprobarNumSeriePuntoMedida() {
    var idNumSerie = document.getElementById('inputNumSerie').value;
    console.log(idNumSerie);

    if (idNumSerie) {

        var url = 'http://172.27.120.120/gestin/public/api/numserierepetidos/puntomedida/' + idNumSerie;
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
 ...
 function comprobarNumSeriePuntoMedida2() {
    var idInstalacion = document.getElementById('inputInstalacion').value;
  
    if (idInstalacion) {

        var url = 'http://172.27.120.120/gestin/public/api/numserierepetidos/puntomedida';
     fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
function comprobarNumSeriePuntoMedida3(id,idNumSerie) {
