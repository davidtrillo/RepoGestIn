function nuevaCamIP() { //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var tipoInstalacion=document.getElementById('inputTipologia').value;
    ...
    if (idInstalacion.value != "" && tipoInstalacion.value == "CÁMARAS"  ) {
        ...
        if (validarFormatoFechaCamIP(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFechaCamIP(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
...
console.log(activo);
        ...
        var url = 'http://172.27.120.111/gestin/public/api/CamIP/nueva';
...
function validarFormatoFechaCamIP(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFechaCamIP(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha2CamIP(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
function rellenarTipoActuacion2CamIP(idActuacion) { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p2.innerHTML += `
             <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion2CamIP(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
             `
            }
        })
}

function rellenarTipoActuacionCamIP() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    var url = 'http://172.27.120.111/gestin/public/api/tipoactuacion'
    fetch(url, {
            method: 'GET',
            headers: {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p.innerHTML += `
             <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacionCamIP(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
             `
            } //CAMBIO DE NOMENCLATURA
        })
}

function leerTipoActuacionCamIP(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion2CamIP(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
...
function rellenarTodosCamIP() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.111/gestin/public/api/CamIP/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
                ...
                for (var i in response) {
                 ...
                 <div class="col-2 mt-1" >
                        <div class="input-group">
                            <button type="button" class="btn btn-secondary dropdown-toggle" name="btnTipoActuacion${response[i]['id']}" value="${response[i]['id']}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onclick="rellenarTipoActuacion2CamIP(this.value)">
                                    Tipo A.
                            </button>
                 ...
                 <div class="col-1">
                    ...
                    <div class="btn btn-danger" id="${response[i]['id']}" onclick="borrarCamIP(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
        ...
        rellenarFooterCamIP();//CAMBIO DE NOMENCLATURA
}

function rellenarFooterCamIP(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.111/gestin/public/api/CamIP/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>CamIP</b> Activas: ${response[0]['c']}</span>
                `
            }
        })
...
function borrarCamIP(param) {
    ...
    var url = 'http://172.27.120.111/gestin/public/api/CamIP/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editarCamIP(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFechaCamIP(inputFechaActuacionTar)) {
        if (existeFechaCamIP(inputFechaActuacionTar)) {
    ...
    var url = 'http://172.27.120.111/gestin/public/api/CamIP/modificar/' + param;
