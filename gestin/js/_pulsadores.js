function nuevaPulsadores() { //CAMBIO DE NOMENCLATURA
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFechaPulsadores(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFechaPulsadores(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
        ...
        var url = 'http://172.27.120.120/gestin/public/api/pulsadores/nueva';
...
function validarFormatoFechaPulsadores(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFechaPulsadores(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha2Pulsadores(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p2.innerHTML += `
             <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion2Pulsadores(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
             `
            }
        })
}
...
function rellenarTipoActuacionPulsadores() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p.innerHTML += `
             <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacionPulsadores(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
             `
            } //CAMBIO DE NOMENCLATURA
        })
}

function leerTipoActuacionPulsadores(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion2Pulsadores(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
...
async function formPulsadores(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    if (instalacion.value != "") {
        ...
        `
  Pulsadores();//CAMBIO DE NOMENCLATURA
...
function rellenarTodosPulsadores() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/pulsadores/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
                ...
                for (var i in response) {
                 ...
                 <div class="col-2 mt-1" >
                        <div class="input-group">
                            <button type="button" class="btn btn-secondary dropdown-toggle" name="btnTipoActuacion${response[i]['id']}" value="${response[i]['id']}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onclick="rellenarTipoActuacion2Pulsadores(this.value)">
                                    Tipo A.
                            </button>
                 ...
                 <div class="col-1">
                    ...
                    <div class="btn btn-danger" id="${response[i]['id']}" onclick="borrarPulsadores(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
        ...
        rellenarFooterPulsadores();//CAMBIO DE NOMENCLATURA
}

function rellenarFooterPulsadores(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/pulsadores/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
...
function borrarPulsadores(param) {
    ...
    var url = 'http://172.27.120.120/gestin/public/api/pulsadores/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editarPulsadores(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFechaPulsadores(inputFechaActuacionTar)) {
        if (existeFechaPulsadores(inputFechaActuacionTar)) {
    ...
    var url = 'http://172.27.120.120/gestin/public/api/pulsadores/modificar/' + param;
