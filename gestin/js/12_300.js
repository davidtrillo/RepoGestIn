function nueva12_300() { //CAMBIO DE NOMENCLATURA
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFecha12_300(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFecha12_300(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
...
activo = String(activo);
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/12_300/nueva';
    ...
    setTimeout(() => {
      rellenarTodos12_300(); //CAMBIO DE NOMENCLATURA
    }, 1000);

}

function validarFormatoFecha12_300(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFecha12_300(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha212_300(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
// function rellenarTipoActuacion212_300(idActuacion) { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
...
//                 p2.innerHTML += `
//              <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion212_300(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
//              `
//             }
//         })
// }

// function rellenarTipoActuacion12_300() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
//     var url = 'http://webserver.mobilitat.local/gestin/public/api/tipoactuacion'
//     fetch(url, {
//             method: 'GET',
//             headers: {
...
//             for (var i in response) {
//                 p.innerHTML += `
//              <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacion12_300(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
//              `
//             } //CAMBIO DE NOMENCLATURA
//         })
// }

function rellenarNID12_300() { //NID
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                ...
                if (str.substring(9,14)=='12300'){// CAMBIAR TIPO DE CARCASA
                p.innerHTML += `
                    <button class="dropdown-item" type="submit" id="dropBtnNID${[i]}" name="${response[i]['nid']}" onclick="leerNID12_300(this.name)">${response[i]['nid']}</button> 
                    `
                }
            }
        })
...
function leerNID12_300(NID) { //NID
    var p1 = document.getElementById('inputNID');
    p1.value = NID;
}

function escribirTipoActuacion12_300(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion212_300(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
    var p2 = document.getElementById('inputTipoActuacion2' + idActuacion);
    p2.value = descripcionTipoActuacion;
}

function rellenarTipoActuacion212_300(descripcionTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA

    var p2 = document.getElementById('inputTipoActuacionTar' + idActuacion);
    p2.value = descripcionTipoActuacion;
}

async function form12_300(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    tipoActuacion.forEach(function(value,index){ //recorrer la matriz de la tabla en tablas.js
        tipo += '<button class="dropdown-item" type="submit" value="'+ value +'" onclick="escribirTipoActuacion12_300(this.value)" >'+ value +'</button>';
    ...
    if (instalacion.value != "") {
        ...
        <div class="row mt-1 pl-1" id="formGuardar">
                ...
                <div class="col-1 pl-1">
                    <input type="text" class="form-control mt-1" name="inputNumSerie" id="inputNumSerie" onfocusout="comprobarNumSerie12_300()">
                </div>
                <div class="col-1 pl-1">
                ...
                <div class="col-1 pl-1">
                <div class="btn btn-primary mt-0 ml-3" onclick="nueva12_300()"><i class="fas fa-save"></i></div>
            </div>
        </div>  
        <!-- Fin Form Introducir nuevo -->
        
        `
    // await rellenarTipoActuacion12_300();
     await rellenarNID12_300();//NID
     await rellenarTodos12_300();//rellenar todos
    }
}



function rellenarTodos12_300() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_300/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            ...
            if (response == "No se han encontrado resultados") {
                ...
                for (var i in response) {
                    ...
                    tipoActuacion.forEach(function(value,index){ //recorrer la matriz de la tabla en tablas.js
                        ...
                        tipo += '<button class="dropdown-item" type="submit" value="'+ value +'" name="'+ response[i]['id'] +'" onclick="rellenarTipoActuacion212_300(this.value,this.name)" >'+ value +'</button>';
             ...
             <div class="row ml-0 pl-0" id="">
                 ...
                 <div class="col-1 pl-0">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editar12_300(this.id)"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" id="${response[i]['id']}" onclick="borrar12_300(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
        ...
        rellenarFooter12_300();//CAMBIO DE NOMENCLATURA
        comprobarNumSerie12_3002();
}

function rellenarFooter12_300(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_300/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>12_300</b> Activos: ${response[0]['c']}</span>
                `
            }
        })
...
function borrar12_300(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_300/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editar12_300(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFecha12_300(inputFechaActuacionTar)) {
        if (existeFecha12_300(inputFechaActuacionTar)) {
    ...
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_300/modificar/' + param;
...
function comprobarNumSerie12_300() {
    var idNumSerie = document.getElementById('inputNumSerie').value;

    if (idNumSerie) {

        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/12_300/' + idNumSerie;
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
function comprobarNumSerie12_3002() {
    var idInstalacion = document.getElementById('inputInstalacion').value;

    if (idInstalacion) {

        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/12_300';
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                ...
                if ((response.length > 0)) {
                    ...
                    for (i in response) {
                        ...
                        if (response[i]['idInstalacion'] == idInstalacion) {
                            ...
                            if (clase) {
                                comprobarNumSerie12_3003(response[i]['id'],response[i]['idNumSerie']);
...
function comprobarNumSerie12_3003(id,idNumSerie) {
