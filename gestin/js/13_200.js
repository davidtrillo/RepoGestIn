function nueva13_200() { //CAMBIO DE NOMENCLATURA
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFecha13_200(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFecha13_200(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
...
activo = String(activo);
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/13_200/nueva';
    ...
    setTimeout(() => {
      rellenarTodos13_200(); //CAMBIO DE NOMENCLATURA
    }, 1000);

}

function validarFormatoFecha13_200(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFecha13_200(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha213_200(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
// function rellenarTipoActuacion213_200(idActuacion) { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
...
//                 p2.innerHTML += `
//              <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion213_200(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
//              `
//             }
//         })
// }

// function rellenarTipoActuacion13_200() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
//     var url = 'http://webserver.mobilitat.local/gestin/public/api/tipoactuacion'
//     fetch(url, {
//             method: 'GET',
//             headers: {
...
//             for (var i in response) {
//                 p.innerHTML += `
//              <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacion13_200(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
//              `
//             } //CAMBIO DE NOMENCLATURA
//         })
// }

function rellenarNID13_200() { //NID
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                ...
                if (str.substring(9,14)=='13200'){// CAMBIAR TIPO DE CARCASA
                p.innerHTML += `
                    <button class="dropdown-item" type="submit" id="dropBtnNID${[i]}" name="${response[i]['nid']}" onclick="leerNID13_200(this.name)">${response[i]['nid']}</button> 
                    `
                }
            }
        })
...
function leerNID13_200(NID) { //NID
    var p1 = document.getElementById('inputNID');
    p1.value = NID;
}

function escribirTipoActuacion13_200(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion213_200(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
    var p2 = document.getElementById('inputTipoActuacion2' + idActuacion);
    p2.value = descripcionTipoActuacion;
}

function rellenarTipoActuacion213_200(descripcionTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA

    var p2 = document.getElementById('inputTipoActuacionTar' + idActuacion);
    p2.value = descripcionTipoActuacion;
}

async function form13_200(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    tipoActuacion.forEach(function(value,index){ //recorrer la matriz de la tabla en tablas.js
        tipo += '<button class="dropdown-item" type="submit" value="'+ value +'" onclick="escribirTipoActuacion13_200(this.value)" >'+ value +'</button>';
    ...
    if (instalacion.value != "") {
        ...
        <div class="row mt-1 pl-1" id="formGuardar">
                ...
                <div class="col-1 pl-1">
                <div class="btn btn-primary mt-0 ml-3" onclick="nueva13_200()"><i class="fas fa-save"></i></div>
            </div>
        </div>  
        <!-- Fin Form Introducir nuevo -->
        
        `
    // await rellenarTipoActuacion13_200();
     await rellenarNID13_200();//NID
     await rellenarTodos13_200();//rellenar todos
    }
}



function rellenarTodos13_200() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/13_200/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            ...
            if (response == "No se han encontrado resultados") {
                ...
                for (var i in response) {
                    ...
                    tipoActuacion.forEach(function(value,index){ //recorrer la matriz de la tabla en tablas.js
                        ...
                        tipo += '<button class="dropdown-item" type="submit" value="'+ value +'" name="'+ response[i]['id'] +'" onclick="rellenarTipoActuacion213_200(this.value,this.name)" >'+ value +'</button>';
             ...
             <div class="row ml-0 pl-0" id="">
                 ...
                 <div class="col-1 pl-0">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editar13_200(this.id)"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" id="${response[i]['id']}" onclick="borrar13_200(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
...
function rellenarFooter13_200(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/13_200/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
...
function borrar13_200(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://webserver.mobilitat.local/gestin/public/api/13_200/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editar13_200(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFecha13_200(inputFechaActuacionTar)) {
        if (existeFecha13_200(inputFechaActuacionTar)) {
    ...
    var url = 'http://webserver.mobilitat.local/gestin/public/api/13_200/modificar/' + param;
...
function comprobarNumSerie13_200() {
    ...
    if (idNumSerie) {
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/13_200/' + idNumSerie;
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
function comprobarNumSerie13_2002() {
    ...
    if (idInstalacion) {
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/13_200';
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
