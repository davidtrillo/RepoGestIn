function nuevaBajantes() { //CAMBIO DE NOMENCLATURA
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFechaBajantes(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFechaBajantes(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
...
activo = String(activo);
    ...
    setTimeout(() => {
        rellenarTodosBajantes(); //CAMBIO DE NOMENCLATURA
    }, 1000);

}

function validarFormatoFechaBajantes(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFechaBajantes(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha2Bajantes(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p2.innerHTML += `
             <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion2Bajantes(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
             `
            }
        })
}
...
function rellenarTipoActuacionBajantes() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p.innerHTML += `
             <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacionBajantes(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
             `
            } //CAMBIO DE NOMENCLATURA
        })
}

function leerTipoActuacionBajantes(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion2Bajantes(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
...
async function formBajantes(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    if (instalacion.value != "") {
        ...
        <div class="col-1">
            <div class="btn btn-primary" onclick="nuevaBajantes()">Guardar</div>
        </div>
        </div>  
        <!-- Fin Form Introducir nuevo -->
        
        `
        await rellenarTipoActuacionBajantes();//CAMBIO DE NOMENCLATURA

        // rellenar todos los registros 
        await rellenarTodosBajantes();//CAMBIO DE NOMENCLATURA
    }
}
...
function rellenarTodosBajantes() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    ...
    fetch(url, {
                ...
                for (var i in response) {
                 ...
                 <div class="col-2 mt-1" >
                        <div class="input-group">
                            <button type="button" class="btn btn-secondary dropdown-toggle" name="btnTipoActuacion${response[i]['id']}" value="${response[i]['id']}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onclick="rellenarTipoActuacion2Bajantes(this.value)">
                                    Tipo A.
                            </button>
                 ...
                 <div class="col-1">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editarBajantes(this.id)" title="Guardar edición"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" title="Eliminar registro" id="${response[i]['id']}" onclick="borrarBajantes(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
        ...
        rellenarFooterBajantes();//CAMBIO DE NOMENCLATURA
}
    ...
    fetch(url, {
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>Bajantes</b> Activas: ${response[0]['c']}</span>
                `
            }
        })
...
function editarBajantes(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFechaBajantes(inputFechaActuacionTar)) {
        if (existeFechaBajantes(inputFechaActuacionTar)) {
