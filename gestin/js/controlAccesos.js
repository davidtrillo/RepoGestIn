...
function nuevaControlAccesos() {
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFechaControlAccesos(fechaActuacion)) {
            if (existeFechaControlAccesos(fechaActuacion)) {
    ...
    setTimeout(() => {
        rellenarTodosControlAccesos();
    }, 1000);

}

function validarFormatoFechaControlAccesos(campo) {
...
function existeFechaControlAccesos(fecha) {
...
function existeFecha2ControlAccesos(fecha) {
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
async function formControlAccesos(elemento) {
    ...
    if (instalacion.value != "") {
        ...
        <div class="col-1">
            <input type="text" class="form-control mt-1" name="inputNumSerie" id="inputNumSerie" onfocusout="comprobarNumSerieControlAccesos()">
        </div>
        <div class="col-1">
        ...
        <div class="col-1">
            <div class="btn btn-primary" onclick="nuevaControlAccesos()">Guardar</div>
        </div>
        </div>  
        ...
        await rellenarTodosControlAccesos();
    }
}



async function rellenarTodosControlAccesos() { //Llamada a la API 
    var idInstalacion = document.getElementById('inputInstalacion').value;

    var url = 'http://172.27.120.120/gestin/public/api/controlaccesos/' + idInstalacion
  await  fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            ...
            if (response == "No se han encontrado resultados") {
              ...
              //  console.log("residuos:"+response[i]['residuos'].value);
                ...
                for (var i in response) {
                 ...
                 <div class="col-1">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editarControlAccesos(this.id)" title="Guardar edición"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" title="Eliminar registro" id="${response[i]['id']}" onclick="borrarControlAccesos(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
   ...
   await rellenarFooterControlAccesos();
   await comprobarNumSerieControlAccesos2();


}

function rellenarFooterControlAccesos() {
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/controlaccesos/instaladas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>ControlAccesos</b> Instaladas: ${response[0]['c']}</span>
                `
            }
        })
...
function borrarControlAccesos(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://172.27.120.120/gestin/public/api/controlaccesos/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editarSimpleControlAccesos(param) {
    ...
    //validar fecha correcta
    if (validarFormatoFechaControlAccesos(inputFechaActuacionTar)) {
        if (existeFechaControlAccesos(inputFechaActuacionTar)) {
    ...
    var url = 'http://172.27.120.120/gestin/public/api/controlaccesos/modificar/' + param;
...
function comprobarNumSerieControlAccesos() {
    var idNumSerie = document.getElementById('inputNumSerie').value;
    console.log(idNumSerie);

    if (idNumSerie) {

        var url = 'http://172.27.120.120/gestin/public/api/numserierepetidos/controlaccesos/' + idNumSerie;
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
async function editarControlAccesos(param) {
    ...
    if  (document.getElementById('inputAlmacenControlTar' + param).checked){
        ...
        if  (document.getElementById('inputResiduosControlTar' + param).checked){
            ...
            editarSimpleControlAccesos(param);
        }
 ...
 function comprobarNumSerieControlAccesos2() {
    var idInstalacion = document.getElementById('inputInstalacion').value;
  
    if (idInstalacion) {

        var url = 'http://172.27.120.120/gestin/public/api/numserierepetidos/controlaccesos';
     fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
function comprobarNumSerieControlAccesos3(id,idNumSerie) {
...
function checkControlInstalada(id) {
    ...
    if (id){
        ...
        if (document.getElementById('inputInstaladaControl').checked) {
