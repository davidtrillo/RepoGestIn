    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                ...
                if (str.substring(3,5)=='0A'){
                p.innerHTML += `
                    <button class="dropdown-item" type="submit" id="dropBtnNID${[i]}" name="${response[i]['nid']}" onclick="leerNID12_pp(this.name)">${response[i]['nid']}</button> 
                    `
                } 
                if (str.substring(2,5)=='00A'){
                    p.innerHTML += `
                        <button class="dropdown-item" type="submit" id="dropBtnNID${[i]}" name="${response[i]['nid']}" onclick="leerNID12_pp(this.name)">${response[i]['nid']}</button> 
                        `
                }
...
async function form12_pp(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    if (instalacion.value != "") {
        ...
        <div class="row mt-1 pl-1" id="formGuardar">
                ...
                <div class="col-1 pl-1">
                <div class="btn btn-primary mt-0 ml-3" onclick="nueva12_pp()"><i class="fas fa-save"></i></div>
            </div>
        </div>  
        <!-- Fin Form Introducir nuevo -->
        
        `
     await rellenarTipoActuacion12_pp();
     await rellenarNID12_pp();//NID
     await rellenarTodos12_pp();//rellenar todos
    }
}
