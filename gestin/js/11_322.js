function nueva11_322() { //CAMBIO DE NOMENCLATURA
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFecha11_322(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFecha11_322(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
...
// console.log(activo);
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/11_322/nueva';
...
function validarFormatoFecha11_322(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFecha11_322(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha211_322(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
function rellenarTipoActuacion211_322(idActuacion) { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p2.innerHTML += `
             <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion211_322(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
             `
            }
        })
}

function rellenarTipoActuacion11_322() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    var url = 'http://webserver.mobilitat.local/gestin/public/api/tipoactuacion'
    fetch(url, {
            method: 'GET',
            headers: {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p.innerHTML += `
             <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacion11_322(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
             `
            } //CAMBIO DE NOMENCLATURA
        })
}

function leerTipoActuacion11_322(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion211_322(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
...
function form11_322(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    if (instalacion.value != "") {
        ...
        <div class="col-1">
            <input type="text" class="form-control mt-1" name="inputNumSerie" id="inputNumSerie"  onfocusout="comprobarNumSerie11_322()">
        </div>
        <div class="col-1">
        ...
        <div class="col-1">
            <div class="btn btn-primary" onclick="nueva11_322()">Guardar</div>
        </div>
        </div>  
        <!-- Fin Form Introducir nuevo -->
        
        `
        rellenarTipoActuacion11_322();//CAMBIO DE NOMENCLATURA
...
function rellenarTodos11_322() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/11_322/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
                ...
                for (var i in response) {
                 ...
                 <div class="col-2 mt-1" >
                        <div class="input-group">
                            <button type="button" class="btn btn-secondary dropdown-toggle" name="btnTipoActuacion${response[i]['id']}" value="${response[i]['id']}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onclick="rellenarTipoActuacion211_322(this.value)">
                                    Tipo A.
                            </button>
                 ...
                 <div class="col-1">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editar11_322(this.id)"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" id="${response[i]['id']}" onclick="borrar11_322(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
        ...
        comprobarNumSerie11_3222();
}

function rellenarFooter11_322(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/11_322/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>11_322</b> Activas: ${response[0]['c']}</span>
                `
            }
        })
...
function borrar11_322(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://webserver.mobilitat.local/gestin/public/api/11_322/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editar11_322(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFecha11_322(inputFechaActuacionTar)) {
        if (existeFecha11_322(inputFechaActuacionTar)) {
    ...
    var url = 'http://webserver.mobilitat.local/gestin/public/api/11_322/modificar/' + param;
...
function comprobarNumSerie11_322() {
    var idNumSerie = document.getElementById('inputNumSerie').value;

    if (idNumSerie) {

        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/11_322/' + idNumSerie;
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
function comprobarNumSerie11_3222() {
    var idInstalacion = document.getElementById('inputInstalacion').value;

    if (idInstalacion) {

        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/11_322';
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                ...
                if ((response.length > 0)) {
                    ...
                    for (i in response) {
                        ...
                        if (response[i]['idInstalacion'] == idInstalacion) {
                            ...
                            if (clase) {
                                comprobarNumSerie11_3223(response[i]['id'],response[i]['idNumSerie']);
...
function comprobarNumSerie11_3223(id,idNumSerie) {
