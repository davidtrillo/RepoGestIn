...
function nuevaSeñalesLuminosas() {
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFechaSeñalesLuminosas(fechaActuacion)) {
            if (existeFechaSeñalesLuminosas(fechaActuacion)) {
    ...
    setTimeout(() => {
        rellenarTodosSeñalesLuminosas();
    }, 1000);

}

function validarFormatoFechaSeñalesLuminosas(campo) {
...
function existeFechaSeñalesLuminosas(fecha) {
...
function existeFecha2SeñalesLuminosas(fecha) {
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
async function formSeñalesLuminosas(elemento) {
    ...
    if (instalacion.value != "") {
        ...
        <div class="col-1">
            <input type="text" class="form-control mt-1" name="inputNumSerie" id="inputNumSerie" onfocusout="comprobarNumSerieSeñalesLuminosas()">
        </div>
        <div class="col-1">
        ...
        await rellenarTodosSeñalesLuminosas();
    }
}



async function rellenarTodosSeñalesLuminosas() { //Llamada a la API 
    var idInstalacion = document.getElementById('inputInstalacion').value;

    var url = 'http://172.27.120.120/gestin/public/api/luminosas/' + idInstalacion
  await  fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
   ...
   await rellenarFooterSeñalesLuminosas();
   await comprobarNumSerieSeñalesLuminosas2();


}

function rellenarFooterSeñalesLuminosas() {
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/luminosas/instaladas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>SeñalesLuminosas</b> Instaladas: ${response[0]['c']}</span>
                `
            }
        })
...
function borrarSeñalesLuminosas(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://172.27.120.120/gestin/public/api/luminosas/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editarSeñalesLuminosas(param) {
    ...
    //validar fecha correcta
    if (validarFormatoFechaSeñalesLuminosas(inputFechaActuacionTar)) {
        if (existeFechaSeñalesLuminosas(inputFechaActuacionTar)) {
    ...
    var url = 'http://172.27.120.120/gestin/public/api/luminosas/modificar/' + param;
...
function comprobarNumSerieSeñalesLuminosas() {
 ...
 function comprobarNumSerieSeñalesLuminosas2() {
    var idInstalacion = document.getElementById('inputInstalacion').value;
...
function comprobarNumSerieSeñalesLuminosas3(id,idNumSerie) {
