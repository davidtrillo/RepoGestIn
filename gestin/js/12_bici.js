function nueva12_bici() { //CAMBIO DE NOMENCLATURA
    ...
    if (idInstalacion.value != "") {
        ...
        if (validarFormatoFecha12_bici(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
            if (existeFecha12_bici(fechaActuacion)) { //CAMBIO DE NOMENCLATURA
...
activo = String(activo);
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/12_bici/nueva';
    ...
    setTimeout(() => {
      rellenarTodos12_bici(); //CAMBIO DE NOMENCLATURA
    }, 1000);

}

function validarFormatoFecha12_bici(campo) { //CAMBIO DE NOMENCLATURA
...
function existeFecha12_bici(fecha) { //CAMBIO DE NOMENCLATURA
...
function existeFecha212_bici(fecha) { //CAMBIO DE NOMENCLATURA
    var fechaf = fecha.split("/");
    var d = fechaf[2];
    var m = fechaf[1];
    var y = fechaf[0];
...
function rellenarTipoActuacion212_bici(idActuacion) { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p2.innerHTML += `
             <button class="dropdown-item" type="submit" id="${idActuacion}" name="${response[i]['id']}" onclick="leerTipoActuacion212_bici(this.value,this.name,this.id)" value="${response[i]['descripcion']}" >${response[i]['descripcion']}</button>
             `
            }
        })
}

function rellenarTipoActuacion12_bici() { //Llamada a la API según el dato obtenido del primer combo //CAMBIO DE NOMENCLATURA
    var url = 'http://webserver.mobilitat.local/gestin/public/api/tipoactuacion'
    fetch(url, {
            method: 'GET',
            headers: {
        ...
        .then(response => {
            ...
            for (var i in response) {
                p.innerHTML += `
             <button class="dropdown-item" type="submit" id="dropBtnTipoActuacion${[i]}" name="${response[i]['id']}" onclick="leerTipoActuacion12_bici(this.value,this.name)" value="${response[i]['descripcion']}">${response[i]['descripcion']}</button> 
             `
            } //CAMBIO DE NOMENCLATURA
        })
}

function rellenarNID12_bici() { //NID
    ...
    fetch(url, {
        ...
        .then(response => {
            ...
            for (var i in response) {
                ...
                if (str.substring(3,5)=='BI'){// CAMBIAR TIPO DE CARCASA
                p.innerHTML += `
                    <button class="dropdown-item" type="submit" id="dropBtnNID${[i]}" name="${response[i]['nid']}" onclick="leerNID12_bici(this.name)">${response[i]['nid']}</button> 
                    `
                }
            }
        })
...
function leerNID12_bici(NID) { //NID
    var p1 = document.getElementById('inputNID');
    p1.value = NID;
}

function leerTipoActuacion12_bici(descripcionTipoActuacion, idTipoActuacion) { //CAMBIO DE NOMENCLATURA
...
function leerTipoActuacion212_bici(descripcionTipoActuacion, idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
...
async function form12_bici(elemento) { //CAMBIO DE NOMENCLATURA
    ...
    if (instalacion.value != "") {
        ...
        <div class="row mt-1 pl-1" id="formGuardar">
                ...
                <div class="col-1 pl-1">
                <div class="btn btn-primary mt-0 ml-3" onclick="nueva12_bici()"><i class="fas fa-save"></i></div>
            </div>
        </div>  
        <!-- Fin Form Introducir nuevo -->
        
        `
     await rellenarTipoActuacion12_bici();
     await rellenarNID12_bici();//NID
     await rellenarTodos12_bici();//rellenar todos
    }
}



function rellenarTodos12_bici() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_bici/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
             ...
             <div class="row ml-1" id="">
                 ...
                 <div class="col-2 mt-1 pl-0" >
                        <div class="input-group">
                            <button type="button" class="btn btn-secondary dropdown-toggle" name="btnTipoActuacion${response[i]['id']}" value="${response[i]['id']}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onclick="rellenarTipoActuacion212_bici(this.value)">
                                    Tipo A.
                            </button>
                 ...
                 <div class="col-1 pl-0">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editar12_bici(this.id)"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" id="${response[i]['id']}" onclick="borrar12_bici(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
...
function rellenarFooter12_bici(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_bici/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
...
function borrar12_bici(param) {
    //Llamada a la API según el dato obtenido del primer combo
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_bici/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editar12_bici(param) {//CAMBIO DE NOMENCLATURA
    ...
    //validar fecha correcta
    if (validarFormatoFecha12_bici(inputFechaActuacionTar)) {
        if (existeFecha12_bici(inputFechaActuacionTar)) {
    ...
    var url = 'http://webserver.mobilitat.local/gestin/public/api/12_bici/modificar/' + param;
...
function comprobarNumSerie12_bici() {
    ...
    if (idNumSerie) {
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/12_bici/' + idNumSerie;
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
...
function comprobarNumSerie12_bici2() {
    ...
    if (idInstalacion) {
        ...
        var url = 'http://webserver.mobilitat.local/gestin/public/api/numserierepetidos/12_bici';
        fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
