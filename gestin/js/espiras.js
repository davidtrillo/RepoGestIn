...
almacen = String(almacen);
        ...
        var url = 'http://172.27.120.120/gestin/public/api/espiras/nueva';
...
function escribirTipoActuacionEspiras(descripcionTipoActuacion) { //CAMBIO DE NOMENCLATURA
    var p2 = document.getElementById('inputTipoActuacion');
    p2.value = descripcionTipoActuacion;
}


function leerTipoActuacionEspiras(idTipoActuacion) { //CAMBIO DE NOMENCLATURA
    var p1 = document.getElementById('idTipoActuacion');
    p1.value = idTipoActuacion;
}

function rellenarTipoActuacion2Espiras(idTipoActuacion, idActuacion) { //CAMBIO DE NOMENCLATURA
...
function rellenarTodosEspiras() { //Llamada a la API  //CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/espiras/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                for (var i in response) {
                 ...
                 <div class="col-1">
                    <div class="btn btn-primary" id="${response[i]['id']}" onclick="editarEspiras(this.id)" title="Guardar edición"><i class="fas fa-pencil-alt"></i></div>
                    <div class="btn btn-danger" title="Eliminar registro" id="${response[i]['id']}" onclick="borrarEspiras(this.id)"><i class="fas fa-trash-alt"></i></div>
                 </div>
              </div>  
...
function rellenarFooterEspiras(){//CAMBIO DE NOMENCLATURA
    var idInstalacion = document.getElementById('inputInstalacion').value;
    var url = 'http://172.27.120.120/gestin/public/api/espiras/activas/' + idInstalacion
    fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => {
            if (response == "No se han encontrado resultados") {
                ...
                <span class="ml-1">Total de <b>Señales Espiras</b> Activas: ${response[0]['c']}</span>
                `
            }
        })
...
function borrarEspiras(param) {
    ...
    var url = 'http://172.27.120.120/gestin/public/api/espiras/borrar/' + param
    fetch(url, {
            method: 'DELETE'
...
function editarEspiras(param) {//CAMBIO DE NOMENCLATURA
    ...
    var url = 'http://172.27.120.120/gestin/public/api/espiras/modificar/' + param;
